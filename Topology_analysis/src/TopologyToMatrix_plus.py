...
INPUT_FILES = {
    'day3': os.path.join(DATASETS_DIR, 'Day3_with_behavior_labels_filled.xlsx'),
    'day6': os.path.join(DATASETS_DIR, 'Day6_with_behavior_labels_filled.xlsx'),
    'day9': os.path.join(DATASETS_DIR, 'Day9_with_behavior_labels_filled.xlsx')
}
OUTPUT_FILES = {
    'day3': os.path.join(DATASETS_DIR, 'Day3_topology_matrix_plus.xlsx'),
    'day6': os.path.join(DATASETS_DIR, 'Day6_topology_matrix_plus.xlsx'),
    'day9': os.path.join(DATASETS_DIR, 'Day9_topology_matrix_plus.xlsx')
}
...
def generate_topologies(on_off_df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    for idx, row in on_off_df.iterrows():
        ...
        if len(on_neurons_sorted) < 2:
            continue

        # Generate all possible pairs of active neurons
        for i in range(len(on_neurons_sorted)):
            for j in range(i + 1, len(on_neurons_sorted)):
                neuron1 = on_neurons_sorted[i]
                neuron2 = on_neurons_sorted[j]
...
def save_topology_matrix(
    ...
    save_path: str = OUTPUT_FILES['day3']
) -> None:
    """
...
def main(
    ...
    topology_matrix_path: str = OUTPUT_FILES['day9'],
...
if __name__ == "__main__":
    try:
        excel_file = INPUT_FILES['day3']
        edge_records = main(
            file_path=excel_file,
            should_save_topology_matrix=True,
            topology_matrix_path=OUTPUT_FILES['day3'],
            initial_edges=None
        )
