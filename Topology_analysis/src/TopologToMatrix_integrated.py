"""
Time Topology Plus Analysis Module for Integrated Data

This module analyzes the temporal topology of neuron activity patterns using integrated data by:
1. Reading calcium concentration data from integrated Excel file
...
INTEGRATED_DATA = os.path.join(DATASETS_DIR, 'integrated_data.xlsx')
OUTPUT_FILE = os.path.join(DATASETS_DIR, 'integrated_topology_matrix.xlsx')
...
def generate_topologies(on_off_df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    All neurons that are ON at the same timestamp will be fully connected to each other.

    Args:
        on_off_df: DataFrame containing ON/OFF states, stamp column, and optionally behavior column

    Returns:
        List of dictionaries containing edge information
    """
    edge_records = []
    neurons = [col for col in on_off_df.columns if col not in ['stamp', 'behavior']]
    sorted_neurons = sorted(neurons, key=extract_index)

    for idx, row in on_off_df.iterrows():
        time_stamp = int(row['stamp'])
        on_neurons = [neuron for neuron in sorted_neurons if row[neuron] == 1]
        
        if len(on_neurons) < 2:
            continue

        # Generate all possible pairs of active neurons
        for i in range(len(on_neurons)):
            for j in range(i + 1, len(on_neurons)):
                neuron1 = on_neurons[i]
                neuron2 = on_neurons[j]
                edge_sorted = tuple(sorted([neuron1, neuron2], key=extract_index))
                record = {
                ...
                # 如果存在behavior列，添加到记录中
                if 'behavior' in row.index:
                    record['behavior'] = row['behavior']
                    
                edge_records.append(record)

        if (idx + 1) % 100 == 0:
            logging.info(f"Generated topology structures for {idx + 1} timestamps")

    logging.info(f"Completed topology structure generation with {len(edge_records)} edges")
    return edge_records
...
def save_topology_matrix(
    ...
    try:
        ...
        # 添加behavior列
        if 'behavior' in edge_df.columns:
            behavior_series = edge_df.groupby('Time_Stamp')['behavior'].first()
            topology_matrix['behavior'] = topology_matrix.index.map(behavior_series)
...
def main(
    file_path: str = INTEGRATED_DATA,
    should_save_topology_matrix: bool = True,
    topology_matrix_path: str = OUTPUT_FILE,
    initial_edges: Optional[List[tuple]] = None
) -> List[Dict[str, Any]]:
    """
    Execute the complete topology analysis workflow for integrated data.

    Args:
        file_path: Path to the integrated Excel file containing calcium data
    ...
    try:
        ...
        # 保持原始数据的stamp和behavior信息
        on_off_df['stamp'] = df['stamp']
        if 'behavior' in df.columns:
            on_off_df['behavior'] = df['behavior']
...
def test_topology_generation():
    ...
    # 生成拓扑结构
    edge_records = generate_topologies(test_df)
    ...
    # 验证时间戳1（n1, n2, n3同时点亮）
    stamp1_edges = edge_df[edge_df['Time_Stamp'] == 1]['Connection'].tolist()
    assert set(stamp1_edges) == set(['n1_n2', 'n1_n3', 'n2_n3']), "时间戳1的连接不完整"
    
    # 验证时间戳2（n3, n4同时点亮）
    stamp2_edges = edge_df[edge_df['Time_Stamp'] == 2]['Connection'].tolist()
    assert set(stamp2_edges) == set(['n3_n4']), "时间戳2的连接不正确"
    
    # 验证时间戳3（n1, n2, n3, n4同时点亮）
    stamp3_edges = edge_df[edge_df['Time_Stamp'] == 3]['Connection'].tolist()
    assert set(stamp3_edges) == set(['n1_n2', 'n1_n3', 'n1_n4', 'n2_n3', 'n2_n4', 'n3_n4']), "时间戳3的连接不完整"
    
    logging.info("拓扑生成测试通过：同时点亮的神经元都正确互相连接")
...
if __name__ == "__main__":
    try:
        ...
        # 运行主程序
        edge_records = main(
            file_path=INTEGRATED_DATA,
            should_save_topology_matrix=True,
            topology_matrix_path=OUTPUT_FILE,
            initial_edges=None
        )
        logging.info("Successfully completed topology analysis for integrated data")
    except Exception as e:
