...
def load_effect_sizes(csv_path_or_data):
    ...
    if isinstance(csv_path_or_data, str) and ".csv" in csv_path_or_data:
        ...
        # 目前，如果文件名是 'EMtrace01-3标签版.csv'，则直接使用硬编码的字符串数据
    ...
    return df_melted[['Behavior', 'NeuronID', 'EffectSize']]

def load_neuron_positions(csv_path_or_data, num_neurons=None, random_seed=42):
    """
    加载神经元位置数据。
    
    当位置数据文件不存在或为空时，自动生成随机位置。

    参数:
        csv_path_or_data (str 或 pd.DataFrame): CSV文件路径或已加载的DataFrame。
                                                若为空字符串、None或文件不存在，则生成随机位置。
        num_neurons (int, optional): 当需要生成随机位置时，指定神经元数量。
                                    如果为None，默认生成43个神经元的位置。
        random_seed (int): 随机数种子，用于生成可重复的随机位置。
    ...
    """
    # 检查是否需要生成随机位置
    should_generate_random = False
    
    if csv_path_or_data is None or csv_path_or_data == "":
        should_generate_random = True
        print("位置数据文件路径为空，将生成随机位置。")
    elif isinstance(csv_path_or_data, str):
        if ".csv" in csv_path_or_data:
            # 首先尝试从实际文件读取
            try:
                import os
                if os.path.exists(csv_path_or_data):
                    print(f"从文件加载神经元位置数据: {csv_path_or_data}")
                    df = pd.read_csv(csv_path_or_data)
                    # 检查文件是否为空
                    if df.empty:
                        print(f"位置数据文件 {csv_path_or_data} 为空，将生成随机位置。")
                        should_generate_random = True
                    else:
                        # 文件存在且不为空，使用文件数据
                    ...
                    print(f"位置数据文件 {csv_path_or_data} 不存在，尝试使用硬编码数据...")
                    # 尝试使用硬编码数据
                    if 'EMtrace01_Max_position.csv' in csv_path_or_data:
                        ...
                        print(f"文件不存在且无匹配的硬编码数据，将生成随机位置。")
                        should_generate_random = True
            except Exception as e:
                print(f"读取位置数据文件时出错: {e}，将生成随机位置。")
                should_generate_random = True
        else:
            print("输入不是有效的CSV文件路径，将生成随机位置。")
            should_generate_random = True
    elif isinstance(csv_path_or_data, pd.DataFrame):
        if csv_path_or_data.empty:
            print("输入的DataFrame为空，将生成随机位置。")
            should_generate_random = True
        else:
            df = csv_path_or_data
            # 确保列名正确
    ...
    if should_generate_random:
        return generate_random_neuron_positions(num_neurons, random_seed)
    
    # 如果走到这里，说明有未处理的情况
    raise ValueError("无法处理输入的位置数据，且无法生成随机位置。")


def generate_random_neuron_positions(num_neurons=None, random_seed=42):
    """
    生成随机的神经元位置数据。
    
    使用黄金角螺旋算法生成均匀分布的位置，确保神经元在空间中分布均匀。

    参数:
        num_neurons (int, optional): 神经元数量。如果为None，默认为43。
        random_seed (int): 随机数种子，用于生成可重复的随机位置。
    ...
    if num_neurons is None:
        num_neurons = 43  # 默认神经元数量
    
    print(f"生成 {num_neurons} 个神经元的随机位置（随机种子: {random_seed}）")
    
    # 设置随机种子以确保可重复性
    np.random.seed(random_seed)
    
    # 使用黄金角螺旋算法生成均匀分布的位置
    golden_angle = np.pi * (3. - np.sqrt(5.))  # 黄金角约为2.39996...
    
    positions = []
    for i in range(num_neurons):
        # 黄金角螺旋公式
        theta = golden_angle * i
        radius = np.sqrt((i + 1) / num_neurons)  # 归一化半径
        
        # 转换为笛卡尔坐标
        x = radius * np.cos(theta)
        y = radius * np.sin(theta)
        
        # 将坐标缩放到 [0, 1] 范围并添加少量随机扰动
        # 这样可以在保持螺旋分布的同时增加一些随机性
        scale_factor = 0.4  # 控制整体分布的范围
        noise_factor = 0.05  # 控制随机扰动的强度
        
        x_scaled = 0.5 + scale_factor * x + noise_factor * np.random.normal()
        y_scaled = 0.5 + scale_factor * y + noise_factor * np.random.normal()
    ...
    print(f"随机位置生成完成。位置范围: x=[{df_positions['x'].min():.3f}, {df_positions['x'].max():.3f}], "
          f"y=[{df_positions['y'].min():.3f}, {df_positions['y'].max():.3f}]")
