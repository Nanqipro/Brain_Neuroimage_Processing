...
# output_folder = r'C:\Users\PAN\PycharmProjects\GitHub\python-RA\数据\Day3'
...
# 设置输入和输出文件路径
file_path = r'C:\Users\PAN\PycharmProjects\GitHub\python-RA\数据\Day9\calcium_window_data_separate_sheets_Day9.xlsx'
output_folder = r'C:\Users\PAN\PycharmProjects\GitHub\python-RA\数据\Day9'
output_file = os.path.join(output_folder, 'calcium_window_result_weighted_output_Day9.xlsx')
...
def process_sheets(input_path, output_path):
    ...
    with pd.ExcelWriter(output_path) as writer:
        for sheet_name in xls.sheet_names:
            ...
            # 提取 Neuron 和 Start Time 列
            neuron_column = df.iloc[:, 0]  # 假设第一列是 Neuron
            start_time_column = df.iloc[:, 1]  # 假设第二列是 Start Time
            ...
            result_df = pd.DataFrame(data, columns=['Amplitude', 'Peak', 'Decay Time', 'Rise Time', 'Latency', 'Frequency'])
            result_df['Cluster'] = labels
            result_df.insert(0, 'Start Time', start_time_column.values)  # 插入 Start Time 列
            result_df.insert(0, 'Neuron', neuron_column.values)  # 插入 Neuron 列
