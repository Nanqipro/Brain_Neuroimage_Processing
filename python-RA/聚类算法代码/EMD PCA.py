...
output_file = 'calcium_window_emd_cluster_results_weighted PCA_sheets.xlsx'
...
with pd.ExcelWriter(output_file) as writer:
    ...
    for sheet_name in xls.sheet_names:
        ...
        # 使用 PCA 进行降维处理（假设降至 3 维以平衡效率和信息保留）
        pca = PCA(n_components=3)
        reduced_data = pca.fit_transform(data)

        # 随机初始化质心
        np.random.seed(42)
        initial_centers = reduced_data[np.random.choice(reduced_data.shape[0], n_clusters, replace=False)]

        # 执行基于 EMD 的并行化 K-means 聚类
        labels, centers = emd_kmeans(reduced_data, initial_centers, max_iter=max_iter)
