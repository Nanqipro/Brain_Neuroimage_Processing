...
#
# # 定义数据标签（可编辑）
# data_label = 'Day 6'  # 这里可以修改为您需要的日期标签
#
# # 读取Excel文件
...
#     else:
#         data[i] = data[i] - mean  # 如果标准差为 0，序列中的值都相同，减去均值后所有值为 0
#
# # 使用自定义的标量距离函数
# def scalar_distance(x, y):
#     return abs(x - y)
...
#     plt.text(time_points[-1], signal[-1] + offset, f'聚类 {label}', fontsize=8, verticalalignment='bottom', horizontalalignment='right')
#
# plt.title(f'{data_label} 基于 DTW 距离的钙波片段聚类（z-score 标准化）')
# plt.xlabel('时间')
# plt.ylabel('标准化钙波信号 + 垂直偏移')
# plt.yticks([])  # 隐藏 y 轴刻度
# plt.tight_layout()
# plt.show()
#
...
#         plt.text(time_points[0], mean_waveform[0] + offset, f'聚类 {cluster_num}', fontsize=10, verticalalignment='bottom')
#
# plt.title(f'{data_label} 每个聚类的平均钙波波形（z-score 标准化）')
# plt.xlabel('时间')
# plt.ylabel('标准化钙波信号 + 垂直偏移')
# plt.yticks([])  # 隐藏 y 轴刻度
# plt.legend()
