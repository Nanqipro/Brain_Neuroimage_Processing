...
def extract_features(normalized_data):
    ...
    参数:
        normalized_data (np.ndarray): 归一化后的数据，形状为（时间步数，神经元数）。

    返回:
        features_data (np.ndarray): 特征矩阵，形状为（神经元数， 3）。
    """
    peak = np.max(normalized_data, axis=0)
    std = np.std(normalized_data, axis=0)
    amplitude = peak - np.min(normalized_data, axis=0)
    features_data = np.vstack((peak, std, amplitude)).T  # shape (n_neurons, 3)
...
def compute_manhattan_distance(data):
    ...
    参数:
        data (np.ndarray): 特征数据，形状为（神经元数， 特征数）。
...
def compute_emd_distance(data):
    ...
    for i in tqdm(range(n_neurons), desc="EMD"):
        ...
        for j in range(i + 1, n_neurons):
            ...
            dist = wasserstein_distance(np.arange(len(u)), np.arange(len(v)), u_weights=u_norm, v_weights=v_norm)
            emd_dist[i, j] = dist
            emd_dist[j, i] = dist
...
def apply_kmeans(data, n_clusters):
    ...
    labels = kmeans.fit_predict(data)  # 每行一个样本
    return labels


def apply_dbscan(distance_matrix, eps=0.5, min_samples=5):
    """
    应用DBSCAN聚类算法。
...
def main():
    # 文件路径（请根据实际情况修改）
    filepath = r"C:\Users\PAN\PycharmProjects\GitHub\python-RA\数据\Day6\calcium_data.xlsx"
    ...
    # 特征提取
    print("提取特征（峰值、标准差、幅度）...")
    features_data = extract_features(normalized_data)
    print(f"特征提取完成，特征矩阵形状: {features_data.shape}")

    # 计算距离矩阵
    print("计算曼哈顿距离矩阵...")
    manhattan_dist = compute_manhattan_distance(features_data)

    print("计算Hausdorff距离矩阵...")
    hausdorff_dist = compute_hausdorff_distance(features_data)

    print("计算EMD距离矩阵...")
    emd_dist = compute_emd_distance(features_data)
    ...
    for metric_name, distance_matrix in distance_metrics.items():
        ...
        # KMeans聚类
        print("应用KMeans聚类...")
        km_labels = apply_kmeans(features_data, n_clusters)
        if metric_name == '曼哈顿距离':
            # 传入特征数据，以确保样本数量一致
            km_score = evaluate_clustering(features_data, km_labels, metric='manhattan')
        else:
            # KMeans始终基于欧几里得距离
            km_score = evaluate_clustering(features_data, km_labels, metric='euclidean')
        ...
        db_labels = apply_dbscan(distance_matrix, eps=3, min_samples=3)
