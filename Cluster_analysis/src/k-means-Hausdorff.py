...
"""
K-means clustering with Euclidean distance for neuron calcium metrics data.
This script implements an optimized K-means clustering algorithm and determines
the optimal number of clusters using elbow and silhouette methods.
"""
...
def k_means_euclidean(X: np.ndarray, n_clusters: int, max_iters: int = 100, 
...
@njit
def compute_wcss_numba(X: np.ndarray, labels: np.ndarray, 
                      centroids: np.ndarray) -> float:
    """
    Compute Within-Cluster Sum of Squares using Numba acceleration.
...
def main():
    ...
    try:
        ...
        for k in K:
            ...
            wcss.append(compute_wcss_numba(X_weighted, labels, centroids))
        ...
        final_labels = k_means_euclidean(X_weighted, n_clusters=optimal_k, random_state=0)
        metrics_df['k-means-Hausdorff'] = final_labels
