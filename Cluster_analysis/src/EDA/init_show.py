...
for i in range(1, 64):
    ...
    if column_name in df.columns:  # Only plot if the column exists
        plt.plot(df['stamp'], df[column_name] * scaling_factor + i * 35, label=column_name)
...
plt.xlabel('Stamp')
plt.ylabel('Traces (n1 ~ n62)')
plt.title('Traces with Increased Amplitude')

# Adjust layout and show the plot
plt.tight_layout()
plt.savefig('../../graph/init_traces_amplitude_day6.png')  # Save the figure
plt.show()
