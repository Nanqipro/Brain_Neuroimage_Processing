...
"""
Hierarchical clustering for neuron calcium metrics data.
This script implements agglomerative hierarchical clustering and determines
the optimal number of clusters using silhouette analysis.
"""
...
def find_optimal_clusters(X: np.ndarray, max_clusters: int = 10) -> tuple:
    """
...
def plot_dendrogram(X: np.ndarray, sample_labels: list):
    """Plot hierarchical clustering dendrogram."""
    linked = linkage(X, method='ward')
    
    plt.figure(figsize=(10, 7))
    dendrogram(linked, labels=sample_labels)
...
def main():
    ...
    try:
        ...
        optimal_n_clusters, silhouette_scores = find_optimal_clusters(X_weighted)
        ...
        # Plot dendrogram
        plot_dendrogram(X_weighted, metrics_df.index.tolist())
        ...
        metrics_df['Hierarchical'] = clusterer.fit_predict(X_weighted)
