...
| 数据集 | 图数量 | 类别数 | 领域 | 描述 |
|--------|--------|--------|------|------|
| **MUTAG** | 188 | 2 | 生物化学 | 分子图分类 - 判断化合物致癌性 |
| **PROTEINS** | 1,113 | 2 | 生物 | 蛋白质图分类 - 判断蛋白质是否为酶 |
| **PTC_MR** | 344 | 2 | 生物化学 | 化合物致癌性预测 |
| **ENZYMES** | 600 | 6 | 生物 | 蛋白质图分类 - 酶类型分类 |
| **NCI1** | 4,110 | 2 | 生物化学 | 化合物抗癌活性预测（标准基准） |
| **NCI109** | 4,127 | 2 | 生物化学 | 化合物抗癌活性预测 |
| **IMDB-BINARY** | 1,000 | 2 | 社交网络 | 电影协作网络 - 判断电影类型 |
| **IMDB-MULTI** | 1,500 | 3 | 社交网络 | 电影协作网络 - 判断电影类型（3类） |
| **REDDIT-BINARY** | 2,000 | 2 | 社交网络 | Reddit社区图分类 |
| **DD** | 1,178 | 2 | 生物 | 蛋白质图分类 - 酶/非酶 |

### OGB (Open Graph Benchmark)

| 数据集 | 图数量 | 任务类型 | 领域 | 描述 |
|--------|--------|----------|------|------|
| **ogbg-molhiv** | 41,127 | 二分类 | 药物发现 | 分子图 - HIV抑制剂预测 |
| **ogbg-molpcba** | 437,929 | 多标签分类 | 药物发现 | 分子图 - 生物活性预测（大规模） |
| **ogbg-ppa** | 158,100 | 多分类 | 生物 | 蛋白质-蛋白质相互作用图 |
...
| 阶段 | 数据集 | 原因 |
|------|--------|------|
| 1️⃣ 快速测试 | MUTAG (188图) | 最小数据集，快速验证代码 |
| 2️⃣ 初步实验 | PROTEINS (1,113图) | 中小规模，适合调参 |
| 3️⃣ 标准基准 | NCI1 (4,110图) | 论文常用基准，结果可对比 |
| 4️⃣ 多分类测试 | ENZYMES (600图, 6类) | 测试多分类性能 |
| 5️⃣ 大规模测试 | ogbg-molhiv (41,127图) | 大规模标准基准 |
...
- **快速验证代码**: MUTAG (188图，几秒钟)
- **调参实验**: PROTEINS (1,113图，几分钟)
- **论文对比**: NCI1 (4,110图，标准基准)
- **大规模实验**: ogbg-molhiv (41,127图，需要GPU)
