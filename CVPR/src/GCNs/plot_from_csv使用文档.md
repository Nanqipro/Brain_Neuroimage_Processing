...
从 v2.0 开始，`plot_from_csv.py` 新增了**内存占用分析**功能：

1. **CPU内存占用分析**：绘制不同数据集规模下的峰值CPU内存占用曲线
2. **GPU内存占用分析**：绘制不同数据集规模下的峰值GPU内存占用曲线
...
- `peak_cpu_memory_vs_scale.png`：峰值CPU内存占用 vs 数据集规模
- `peak_gpu_memory_vs_scale.png`：峰值GPU内存占用 vs 数据集规模
