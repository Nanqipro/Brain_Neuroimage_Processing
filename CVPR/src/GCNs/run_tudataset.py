...
使用下载好的图分类数据集（MUTAG, PROTEINS, NCI1等）训练模型
"""
...
def setup_result_directory(model_name, dataset_name):
    """创建结果保存目录"""
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir = f"result/tudataset/{dataset_name}/{model_name}"
...
def load_tudataset(dataset_name, data_root='../../data/TUDataset'):
    """
    加载TUDataset数据集
    
    Args:
        dataset_name: 数据集名称，如 'MUTAG', 'PROTEINS', 'NCI1'
        data_root: 数据集根目录
    ...
    if len(dataset) > 0:
        ...
        print(f"  - 节点特征形状: {sample.x.shape if sample.x is not None else 'None'}")
        print(f"  - 边特征: {'有' if hasattr(sample, 'edge_attr') and sample.edge_attr is not None else '无'}")
...
def run_single_experiment(args):
    ...
    # 加载数据集
    dataset = load_tudataset(args.dataset, args.data_root)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if val_metrics['f1'] > best_val_f1:
            ...
            if args.save_model:
                result_dir = setup_result_directory(args.model, args.dataset)
...
def main():
    parser = argparse.ArgumentParser(description="在TUDataset上训练图分类模型")
    
    # 数据集参数
    parser.add_argument('--dataset', type=str, default='MUTAG',
                        help='数据集名称 (MUTAG, PROTEINS, NCI1, ENZYMES等)')
