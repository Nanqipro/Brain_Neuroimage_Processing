...
# ⚠️ 注意：所有数据集使用统一的batch_size=32以确保实验可比性
DATASETS=(
    # "tudataset:MUTAG:32:300:0"                                                      # GPU 0
    # "ogb:ogbg-molhiv:32:300:1"                                                      # GPU 1
    # "tudataset:PROTEINS:32:300:2"                                                   # GPU 2
    "custom:../../data/random/graphs_100:32:300:1"                                  # GPU 0 - Random 100图 (并行)
    "custom:../../data/random/graphs_1000:32:300:1"                                 # GPU 1 - Random 1000图 (并行)
    "custom:../../data/random/graphs_10000:32:300:1"                                # GPU 2 - Random 10000图 (并行)
    "custom:../../data/random/graphs_100000:32:300:2"                              # GPU 3 - Random 100000图 (并行)
    "custom:../../data/random/graphs_1000000:32:300:3"                             # GPU 3 - Random 1000000图 (顺序执行)
    # "ogb:ogbg-ppa:32:300:3"                                                      # GPU 3 (备用)
)
...
HARDWARE_LOG="$LOG_DIR/hardware_info.log"

# 记录硬件信息
echo "=====================================================================" > "$HARDWARE_LOG"
echo "🖥️  硬件配置信息" >> "$HARDWARE_LOG"
echo "=====================================================================" >> "$HARDWARE_LOG"
echo "记录时间: $(date '+%Y-%m-%d %H:%M:%S')" >> "$HARDWARE_LOG"
echo "" >> "$HARDWARE_LOG"

echo "【CPU信息】" >> "$HARDWARE_LOG"
lscpu | grep -E "Model name|CPU\(s\)|Thread|Core|Socket|MHz" >> "$HARDWARE_LOG" 2>&1 || echo "CPU信息获取失败" >> "$HARDWARE_LOG"
echo "" >> "$HARDWARE_LOG"

echo "【内存信息】" >> "$HARDWARE_LOG"
free -h >> "$HARDWARE_LOG" 2>&1 || echo "内存信息获取失败" >> "$HARDWARE_LOG"
echo "" >> "$HARDWARE_LOG"

echo "【GPU信息】" >> "$HARDWARE_LOG"
nvidia-smi --query-gpu=index,name,memory.total,driver_version,compute_cap --format=csv >> "$HARDWARE_LOG" 2>&1 || echo "GPU信息获取失败" >> "$HARDWARE_LOG"
echo "" >> "$HARDWARE_LOG"
nvidia-smi >> "$HARDWARE_LOG" 2>&1 || echo "详细GPU信息获取失败" >> "$HARDWARE_LOG"
echo "=====================================================================" >> "$HARDWARE_LOG"

echo "✅ 硬件信息已记录到: $HARDWARE_LOG"
echo ""
...
echo "统一超参数: batch_size=32, epochs=300, lr=0.001"
