...
#
# # 1. 数据读取
# neuron_data = pd.read_excel('./data/Day6.xlsx')
#
# # 获取神经元ID列表
# neuron_ids = neuron_data.columns[1:]
#
...
#         title=f'神经元拓扑结构图 - 时间点：{neuron_data["stamp"].iloc[i]}'
#     )
#
# # 创建初始图形
...
#     layout=go.Layout(
#         title=f'神经元拓扑结构图 - 时间点：{neuron_data["stamp"].iloc[0]}',
#         xaxis=dict(
#             range=x_range,
#             showgrid=False,
#             zeroline=False,
...
#                             'transition': {'duration': 0}
#                         }],
#                         label=str(neuron_data['stamp'].iloc[k]))
#                    for k in range(len(frames))]
#         )]
#     ),
#     frames=frames
...
# 1. 数据读取
neuron_data = pd.read_excel('./data/Day6.xlsx')  # 请修改为您的数据路径
